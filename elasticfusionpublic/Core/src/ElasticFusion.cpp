...
{
	//pht
	textures[GPUTexture::INSTANCE_GROUNDTRUTH] = new GPUTexture(Resolution::getInstance().width(),
																Resolution::getInstance().height(),
																GL_LUMINANCE8UI_EXT,				//internalFormat	->	internal_format
																GL_LUMINANCE_INTEGER_EXT,			//format			->  glformat
																GL_UNSIGNED_BYTE,					//dataType			->  gltype
															    false,								//draw				->	sampling_linear
															    false);						//cuda
...
void ElasticFusion::processFrame(const unsigned char * rgb,
								 ...
								 const unsigned char * instanceGT,
								 ...
								 const bool bootstrap)
{
	...
	//pht
	if(instanceGT!=NULL)
		textures[GPUTexture::INSTANCE_GROUNDTRUTH]->texture->Upload(instanceGT, GL_LUMINANCE_INTEGER_EXT, GL_UNSIGNED_BYTE);
	//debug
	if(false)
	{
		unsigned short * tempImage = new unsigned short[Resolution::getInstance().width()*Resolution::getInstance().height()];
		glBindTexture(GL_TEXTURE_2D, textures[GPUTexture::INSTANCE_GROUNDTRUTH]->texture->tid);
		glGetTexImage(GL_TEXTURE_2D, 0, GL_LUMINANCE_INTEGER_EXT, GL_UNSIGNED_BYTE, tempImage);
		glBindTexture(GL_TEXTURE_2D, 0);
		cv::Mat inst_image(Resolution::getInstance().height(),Resolution::getInstance().width(),CV_8UC1, tempImage);
		cv::imwrite("./temp/instance.png",inst_image);
	}
	if(false)
	{
		unsigned short * tempImage = new unsigned short[Resolution::getInstance().width()*Resolution::getInstance().height()];
		glBindTexture(GL_TEXTURE_2D, textures[GPUTexture::DEPTH_RAW]->texture->tid);
		glGetTexImage(GL_TEXTURE_2D, 0, GL_LUMINANCE_INTEGER_EXT, GL_UNSIGNED_SHORT, tempImage);
		glBindTexture(GL_TEXTURE_2D, 0);
		cv::Mat depth_image(Resolution::getInstance().height(),Resolution::getInstance().width(),CV_16UC1, tempImage);
		cv::imwrite("./temp/depth1.png",depth_image);
	}
	...
	//pht add===================================
	//frameID == tick-1
	globalModel.updateFrameID(tick);
	//pht add===================================
	...
	//================================================================================================================
		...
		if(closeLoops && ferns.lastClosest != -1)
		{
			...
			{
				//std::cout<<"tick:"<<tick<<" Do GlobalDeformation"<<std::endl;
		...
		if(!lost && closeLoops && rawGraph.size() == 0)
		{
			...
			//true -> localDeformation ON
			if(true && covOk && modelToModel.lastICPCount > icpCountThresh && modelToModel.lastICPError < icpErrThresh)
			{
				...
				//std::cout<<"tick:"<<tick<<"Do LocalDeformation"<<std::endl;
	...
	//================================================================================================================
		if(!trackingOnly && !rgbOnly && trackingOk && !lost)
		{
			...
			else
			{
				globalModel.fuse(currPose,
								 ...
								 textures[GPUTexture::DEPTH_METRIC],
								 textures[GPUTexture::DEPTH_METRIC_FILTERED],
								 textures[GPUTexture::INSTANCE_GROUNDTRUTH],
								 indexMap.indexTex(),
								 indexMap.vertConfTex(),
								 indexMap.colorTimeTex(),
								 indexMap.normalRadTex(),
		...
		//==========For Instance=======================================
			indexMap.renderSurfelIds(currPose, tick, globalModel.model(), confidenceThreshold, maxDepthProcessed, timeDelta,IndexMap::INSTANCECOMPARE);
			indexMap.renderSurfelIds(currPose, tick, globalModel.model(), confidenceThreshold, maxDepthProcessed, timeDelta,IndexMap::GENERAL_BEFORE);
		//=============================================================
			globalModel.clean(currPose,
							  ...
							  maxDepthProcessed,
							  fernAccepted);
			indexMap.renderSurfelIds(currPose, tick, globalModel.model(), confidenceThreshold, maxDepthProcessed, timeDelta,IndexMap::GENERAL_AFTER);
		} else if (trackingOnly) {
			indexMap.renderSurfelIds(currPose, tick, globalModel.model(), confidenceThreshold, maxDepthProcessed, timeDelta,IndexMap::GENERAL_AFTER);	//BEFORE=AFTER
		}
	}
...
{
	TICK("Ferns::addFrame");
	ferns.addFrame(&fillIn.imageTexture, &fillIn.vertexTexture, &fillIn.normalTexture,indexMap.instTex(), currPose, tick, fernThresh);
	TOCK("Ferns::addFrame");
}
...
void ElasticFusion::savePly()
{
	...
	std::cout<<"write ply file : "<<filename<<std::endl;
	// Open file
	std::ofstream fs;
	...
	{
		Eigen::Vector4f pos = mapData[(i * 16) + 0];
	...
	for(unsigned int i = 0; i < globalModel.lastCount(); i++)
	{
		...
		if(pos[3] > confidenceThreshold)
		{
			Eigen::Vector4f col = mapData[(i * 16) + 1];
			Eigen::Vector4f nor = mapData[(i * 16) + 2];
	...
	//===============================================================
	//lyc add : to output Instance model
	filename = saveFilename;
	filename.append("_Instance.ply");
	...
	for(unsigned int i = 0; i < globalModel.lastCount(); i++)
	{
		...
		if(pos[3] > confidenceThreshold)
		{
			...
			unsigned char r = int(col[1]) >> 16 & 0xFF;
			unsigned char g = int(col[1]) >> 8 & 0xFF;
			unsigned char b = int(col[1]) & 0xFF;
	...
	//===============================================================
	//pht add : to output instanceGT model
	filename = saveFilename;
	filename.append("_InstanceGT.ply");
	...
	for(unsigned int i = 0; i < globalModel.lastCount(); i++)
	{
		...
		if(pos[3] > confidenceThreshold)
		{
			Eigen::Vector4f col = mapData[(i * 16) + 1];
			Eigen::Vector4f nor = mapData[(i * 16) + 2];
			Eigen::Vector4f ico = mapData[(i * 16) + 3];
			...
			unsigned char r = int(ico[3])*3%255;
			unsigned char g = int(ico[3])*17%255;
			unsigned char b = int(ico[3])*101%255;
	...
	fs << "\nproperty float time1";
	fs << "\nproperty float time2";


	fs << "\nproperty float ImgCorr1";
	fs << "\nproperty float ImgCorr2";
	fs << "\nproperty float ImgCorr3";
	fs << "\nproperty float ImgCorr4";
	...
	for(unsigned int i = 0; i < globalModel.lastCount(); i++)
	{
		...
		fs << value << "\t";
			...
			memcpy (&value, &ico[0], sizeof (float));
		fs << value << "\t";
			memcpy (&value, &ico[1], sizeof (float));
		fs << value << "\t";
			memcpy (&value, &ico[2], sizeof (float));
		fs << value << "\t";
			memcpy (&value, &ico[3], sizeof (float));
		fs << value << "\n";
		}
	}
